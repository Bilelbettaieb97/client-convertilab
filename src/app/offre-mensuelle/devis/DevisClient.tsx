...
const sectors = [
  ...
  { value: "beaute", label: "Beaute / Bien-etre", icon: Scissors, emoji: "✂️" },
  { value: "sante", label: "Sante", icon: Stethoscope, emoji: "🩺" },
  ...
  { value: "photo", label: "Photo / Creatif", icon: Camera, emoji: "📸" },
...
export default function DevisClient() {
  ...
  useEffect(() => {
    ...
    if (offreParam) {
      setFormData((prev) => ({ ...prev, offer: offreParam }));
    }
  }, []);
  ...
  const handleNext = () => {
    if (!canContinue()) {
      if (step === 0) toast({ title: "Selectionnez votre secteur d'activite", variant: "destructive" });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: "Erreur, veuillez reessayer", variant: "destructive" });
    } finally {
  ...
  const stepLabels = ["Activite", "Offre", "Coordonnees"];
        ...
        <div className="container mx-auto px-4 max-w-lg">
          ...
          <div className="text-center mb-6 animate-fade-in">
            ...
            <p className="text-sm text-muted-foreground">
              Reponse personnalisee sous 24h - Sans engagement
            </p>
          </div>
          ...
          {step <= totalSteps && step < 3 && (
            <div className="mb-6">
                ...
                {stepLabels.map((label, i) => (
                  <span key={i} className={`flex items-center gap-1 ${i <= step ? "text-primary font-medium" : ""}`}>
                    {i < step ? (
                      ...
                      <span className={`w-4 h-4 rounded-full text-[9px] flex items-center justify-center font-bold ${i === step ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                        {i + 1}
                      </span>
                    )}
                    {label}
              ...
              <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-primary to-pink-500 rounded-full transition-all duration-700 ease-out" style={{ width: `${progress}%` }} />
              </div>
            </div>
          )}
            ...
            <div className="p-5 sm:p-6">
              ...
              {step === 0 && (
                <div key="step0" className={animClass}>
                  <h2 className="text-lg font-bold text-foreground mb-1">Votre activite</h2>
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    {sectors.map((s) => {
                      ...
                      return (
                        <button key={s.value} type="button" onClick={() => setFormData({ ...formData, sector: s.value })} className={`relative flex items-center gap-2.5 p-3 rounded-xl transition-all duration-300 border-2 text-left ${active ? "border-primary bg-primary/5 shadow-md" : "border-border bg-card hover:border-primary/30"}`}>
                          <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-all ${active ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                            <Icon className="w-4 h-4" />
                          </div>
                          <span className="text-xs font-medium text-foreground leading-tight">{s.label}</span>
                          {active && <CheckCircle2 className="w-3.5 h-3.5 text-primary absolute top-1.5 right-1.5 animate-scale-in" />}
                        </button>
                      );
                    })}
                  </div>
                  <div className="mt-4">
                    <Textarea placeholder="Decrivez brievement votre activite... (optionnel)" value={formData.companyDescription} onChange={(e) => setFormData({ ...formData, companyDescription: e.target.value })} rows={2} className="bg-muted/50 border-border rounded-xl text-sm focus:ring-2 focus:ring-primary resize-none" />
                  </div>
                </div>
              )}
              ...
              {step === 1 && (
                <div key="step1" className={animClass}>
                  ...
                  <p className="text-xs text-muted-foreground mb-4">Vous etes proprietaire dans les 3 formules</p>
                  ...
                  <div className="space-y-3">
                    {offers.map((offer) => {
                      ...
                      return (
                        <button key={offer.value} type="button" onClick={() => setFormData({ ...formData, offer: offer.value })} className={`w-full text-left p-4 rounded-xl transition-all duration-300 border-2 relative ${active ? "border-primary bg-primary/5 shadow-lg shadow-primary/10" : "border-border bg-card hover:border-primary/30 hover:shadow-md"}`}>
                          {offer.popular && <span className="absolute -top-2.5 right-3 bg-primary text-primary-foreground text-[9px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1"><Crown className="w-3 h-3" /> Populaire</span>}
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <div className="text-right">
                              <span className="text-2xl font-extrabold text-foreground">{offer.price}eur</span>
                          ...
                          <ul className="space-y-1.5">
                            {offer.features.map((feat, i) => (
                              <li key={i} className={`flex items-start gap-2 text-[11px] ${feat.highlight ? "bg-primary/5 -mx-1 px-1 py-0.5 rounded" : ""}`}>
                                <CheckCircle className={`w-3.5 h-3.5 flex-shrink-0 mt-0.5 ${feat.highlight ? "text-primary" : active ? "text-primary" : "text-muted-foreground"}`} />
                                <span className={`${feat.bold ? "font-bold text-primary" : feat.highlight ? "font-semibold text-foreground" : active ? "text-foreground" : "text-muted-foreground"}`}>
                                  {feat.text}
                          ...
                          {offer.savings && <div className="mt-3 text-[10px] font-semibold text-primary bg-primary/10 rounded-full px-3 py-1 inline-block">{offer.savings}</div>}
                          {active && <div className="absolute top-4 right-4"><CheckCircle2 className="w-5 h-5 text-primary animate-scale-in" /></div>}
                        </button>
                      );
                    })}
                  </div>
              ...
              {step === 2 && (
                <div key="step2" className={animClass}>
                  <h2 className="text-lg font-bold text-foreground mb-1">Vos coordonnees</h2>
                  <p className="text-xs text-muted-foreground mb-4">Pour recevoir votre devis personnalise</p>
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="relative group">
                        ...
                        <Input placeholder="Votre nom *" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="pl-9 h-11 text-sm bg-muted/50 border-border rounded-xl focus:ring-2 focus:ring-primary" />
                      </div>
                      <div className="relative group">
                        ...
                        <Input placeholder="Entreprise *" value={formData.company} onChange={(e) => setFormData({ ...formData, company: e.target.value })} className="pl-9 h-11 text-sm bg-muted/50 border-border rounded-xl focus:ring-2 focus:ring-primary" />
                      </div>
                    </div>
                    <div className="relative group">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-focus-within:text-primary transition-colors" />
                      <Input type="email" placeholder="votre@email.com *" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="pl-9 h-11 text-sm bg-muted/50 border-border rounded-xl focus:ring-2 focus:ring-primary" />
                    </div>
                    <div className="relative group">
                      ...
                      <Input type="tel" placeholder="Votre telephone *" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="pl-9 h-11 text-sm bg-muted/50 border-border rounded-xl focus:ring-2 focus:ring-primary" />
                    </div>
                    <Textarea placeholder="Un message ou des precisions ? (optionnel)" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={2} className="bg-muted/50 border-border rounded-xl text-sm focus:ring-2 focus:ring-primary resize-none" />
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div key="step3" className="animate-scale-in text-center py-4">
                  ...
                  <div className="space-y-2.5">
                    <Button type="button" onClick={() => window.open("https://calendly.com/convertilab-5bsc/30min", "_blank")} className="w-full h-12 bg-gradient-to-r from-primary to-pink-500 hover:opacity-90 rounded-xl font-semibold text-sm text-primary-foreground">
                      <Calendar className="mr-2 w-4 h-4" />Reserver un appel
                    </Button>
                    <Button asChild variant="outline" className="w-full h-10 rounded-xl text-sm">
                      <Link href="/offre-mensuelle">Retour aux offres</Link>
                    </Button>
                  </div>
                </div>
              )}
            ...
            {step <= 2 && (
              <div className="px-5 pb-5 pt-1">
                <div className="flex gap-3">
                  {step > 0 && (
                    <Button type="button" variant="outline" onClick={() => go("back")} className="flex-1 h-12 rounded-xl border-2 text-sm">
                      <ArrowLeft className="mr-1.5 w-4 h-4" />Retour
                    </Button>
                  )}
                  <Button type="button" onClick={handleNext} disabled={isSubmitting} className={`${step > 0 ? "flex-1" : "w-full"} h-12 rounded-xl font-semibold text-sm transition-all ${step === 2 ? "bg-gradient-to-r from-primary to-pink-500 text-primary-foreground hover:opacity-90" : "bg-primary text-primary-foreground hover:bg-primary/90"}`}>
                    {isSubmitting ? (
                      <span className="flex items-center"><div className="w-4 h-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin mr-2" />Envoi...</span>
                    ) : step === 2 ? (
                      <><Send className="mr-1.5 w-4 h-4" />Envoyer ma demande</>
                    ) : (
                      <>Continuer<ArrowRight className="ml-1.5 w-4 h-4" /></>
                    )}
                  </Button>
                </div>
                <div className="flex justify-center gap-4 mt-4 text-[10px] text-muted-foreground">
                  <span className="flex items-center gap-1"><Shield className="w-3 h-3 text-primary" /> 100% gratuit</span>
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-primary" /> Reponse 24h</span>
                  <span className="flex items-center gap-1"><Star className="w-3 h-3 text-primary" /> Sans engagement</span>
                </div>
              </div>
            )}
          </div>
